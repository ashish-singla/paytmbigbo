...
class App_Locations_LocationsWorker {
	...
	public function show_appointment_location ($out, $appointment) {
		...
		foreach ($all as $loc) {
			...
			$editable .= '<option value="' . esc_attr($loc->get_id()) . '" ' . $sel . '>' . $loc->get_admin_label() ."-". $loc->to_storage()['area'] . '</option>';
		}
		$editable .= '</select>';
		...
		<div class="inside">
			...
			<?php foreach ($locations as $location) {?>
				...
				<li id="app-location-<?php esc_attr_e($location->get_id()); ?>">
					...
					<b><?php echo $location->to_storage()['area']; ?></b>
