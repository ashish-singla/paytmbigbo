<?php
/*
Plugin Name: Locationscity
Description: Allows you to create locations for your appointments.
Plugin URI: http://premium.wpmudev.org/project/appointments-plus/
Version: 1.0
AddonType: Locationscity
Author: WPMU DEV
*/

class App_Locationscity_LocationsWorker {

	const SETTINGS_TAB = 'locationscity';
	const INJECT_TAB_BEFORE = 'services';

	private $_data;
	private $_locationscity;

	private function __construct () {}

	public static function serve () {
		$me = new App_Locationscity_LocationsWorker;
		$me->_add_hooks();
	}
	...
	private function _add_hooks () {
		...
		// Appointments list
		add_filter('app-appointments_list-edit-services', array($this, 'show_appointment_locationcity'), 10, 2);
		add_filter('app-appointment-inline_edit-save_data', array($this, 'save_appointment_locationcity'));
	}
//appointments location city
	public function save_appointment_locationcity ($data) {
		if (empty($data) || !is_array($data)) return $data;
		$location_id = !empty($_POST['locationcity']) ? $_POST['locationcity'] : false;
		$data['location'] = $location_id;
		return $data;
	}

	public function show_appointment_locationcity($out, $appointment) {
		$editable = '';
		$all = $this->_locationscity->get_all();
		$editable .= '<span class="title">' . __('Locationcity', 'appointments') . '</span>';
...
<form method="post" action="" >
	...
	<div class="postbox">
		...
		<div class="inside">
			...
			<?php } ?>
			<input type="hidden" name="action_app1" value="save_locations1" />
	...
	public function save_settings ($options) {
		if (empty($_POST['action_app1']) || 'save_locations1' != $_POST['action_app1']) return false;
...
App_Locationscity_LocationsWorker::serve();