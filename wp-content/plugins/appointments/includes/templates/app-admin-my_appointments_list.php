...
<?php } ?>
	...
	jQuery(document).ready(function($){
		...
		$("table").on("click", ".save", function(){
			...
			var area = save_parent.find('input[name="area"]').val();
			...
			var resend = 0;
			//if(status=="confirmed"){resend=1;}
			
			if (save_parent.find('input[name="resend"]').is(':checked') ) { resend=1;}
			var app_id = save_parent.find('input[name="app_id"]').val();
			var data = {action: 'inline_edit_save', user:user, name:name, email:email, phone:phone, address:address,city:city, area:area, service:service, worker:worker, price:price, date:date, time:time, note:note, status:status, resend:resend, app_id: app_id, nonce: '<?php echo wp_create_nonce() ?>'};
